...
public class PerguntasNivel1 {
	...
	public static void main(String[] args) {
		...
		String perguntas_Nivel1[][] = new String[5][5];
		
		
		for (int i = 0; i < 5; i++) {
			int linha = indicesLinhas.get(i);
			
			perguntas_Nivel1[i][0] = quiz[linha][0];
			perguntas_Nivel1[i][1] = quiz[linha][1];
			perguntas_Nivel1[i][2] = quiz[linha][2];
			perguntas_Nivel1[i][3] = quiz[linha][3];
			perguntas_Nivel1[i][4] = quiz[linha][4];
			}	
		
		
		//Módulo de Teste - apagar depois
		for (int i = 0; i < 5; i++) {
			System.out.println(indicesLinhas.get(i));
		}
		
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				System.out.println(perguntas_Nivel1[i][j]);
			}
		}
		//Fim do módulo de teste
